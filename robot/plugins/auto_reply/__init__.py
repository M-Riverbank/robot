...
api_switch = on_command("智障回复api切换", aliases={"ai切换", "api_switch", "智能回复api切换"}, permission=SUPERUSER,
...
async def _(event: MessageEvent):
    ...
    if result is None:
        if api_flag:
            ...
            if apiKey == "寄":
                await ai.finish("找不到小爱同学, 请联系主淫在.env中设置")
            message = await xiaoice_reply(xiaoai_url)
            if message == "寄":
                await ai.finish("找不到小爱同学, 请联系主淫在.env中重新设置")
